...
homekit_characteristic_t name = HOMEKIT_CHARACTERISTIC_(NAME, "TempSensor");
...
void user_init(void) {
    ...
    uint8_t macaddr[6];
        ...
        int name_len = snprintf(NULL, 0, "TempSensor-%02X%02X%02X",
                                macaddr[3], macaddr[4], macaddr[5]);
        char *name_value = malloc(name_len+1);
        snprintf(name_value, name_len+1, "TempSensor-%02X%02X%02X",
                 macaddr[3], macaddr[4], macaddr[5]);
